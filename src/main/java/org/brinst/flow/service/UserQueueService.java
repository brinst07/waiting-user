...
public class UserQueueService {
	...
	public Mono<Long> registerWaitQueue(final String queue, final Long userId) {
		//redis sortedSet -> sortedset을 활용하여, userId의 중복을 제거하고, 시간 순서대로 rank기능을 활용
		// - key : userId
	...
	public Mono<Boolean> isAllowedByToken(final String queue, final Long userId, final String token) {
		return generateToke(queue, userId)
	...
	public Mono<String> generateToke(final String queue, final Long userId) {
		// sha256
	...
	public void scheduleAllowUser() {
		...
		reactiveRedisTemplate
			.scan(ScanOptions.scanOptions().match(USER_QUEUE_WAIT_KEY_FOR_SCAN).count(100).build())
			.map(key -> key.split(":")[2])
